...
| stream false / true | native Responses SSE + canonical aggregation | translated | Responses passthrough preserves native response.completed/response.incomplete events; aggregation emits response.completed. No Chat [DONE]. Incomplete events retain usage and terminal classification. | `responses.native.openai.sync`<br>`responses.native.openai.stream`<br>`responses.native.openai.incomplete`<br>`responses.translate.openai.sync`<br>`responses.translate.openai.stream`<br>`responses.translate.gemini.sync`<br>`responses.translate.gemini.stream`<br>`responses.translate.anthropic.sync`<br>`responses.translate.anthropic.stream` |
| refusal content / response.refusal.delta | preserved in native refusal item/event | normalized to canonical text | Responses passthrough preserves native refusal semantics; translated adapters retain user-visible refusal text through the canonical output path. | `responses.native.openai.refusal`<br>`responses.native.openai.stream`<br>`responses.native.openai.passthrough` |
...
| `responses.native.openai.passthrough` | cargo | Responses API end-to-end preserves validated raw fields while rewriting model, store and endpoint |
| `responses.native.openai.stream` | cargo | Native Responses SSE events normalize text, tool calls, usage and terminal reason |
| `responses.native.openai.refusal` | cargo | Native Responses refusal objects and refusal.delta events preserve refusal text |
| `responses.native.openai.incomplete` | cargo | Native response.incomplete events preserve usage and classify the terminal reason |
