...
| stream false / true | native Responses SSE + canonical aggregation | translated | Aggregation emits only recognized Responses incomplete reasons. Refusals remain completed; unknown provider-specific terminal reasons produce a valid Responses error. Chat [DONE] is not emitted. | `responses.native.openai.sync`<br>`responses.native.openai.stream`<br>`responses.native.openai.incomplete`<br>`responses.translate.openai.sync`<br>`responses.translate.openai.stream`<br>`responses.translate.gemini.sync`<br>`responses.translate.gemini.stream`<br>`responses.translate.anthropic.sync`<br>`responses.translate.anthropic.stream`<br>`responses.translate.provider_terminal_reasons` |
| usage input_tokens_details / output_tokens_details | known cached, cache-write and reasoning token breakdowns preserved | canonical usage | Responses aggregation emits the documented plural detail fields, includes cache_write_tokens when supplied, and omits unknown breakdown metadata. | `responses.native.openai.sync`<br>`responses.native.openai.passthrough` |
...
| `responses.translate.provider_terminal_reasons` | cargo | Anthropic refusals map to completed Responses; unsupported provider terminal reasons return status=failed without exposing the provider reason |
