...
| FR-12.5 | Selection strategies after eligibility filtering: `priority`, `round-robin`, `weighted`, `least-used`, or `adaptive`. Adaptive selection uses target-local concurrency plus observed TTFT/error/overload telemetry; sticky/cache-affinity remains a stronger continuity preference as permitted by FR-7. | MUST | 2 |
