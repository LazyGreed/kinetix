...
- **Credential refresh and plugin runtime**:
  - `src/credential_refresh.rs` tests cover proactive rotation, unchanged four-minute lease deadlines, due-only retry floors capped by expiry, and preservation of future explicit and expiry-derived deadlines for short leases.
