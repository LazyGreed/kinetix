...
- **Core Pipeline & Router (`src/pipeline.rs`, `src/router.rs`, `src/registry.rs`)**:
  ...
  - Execution engine: handles upstream target dispatch, credential leasing from pools, streaming-safe failovers, response streaming, usage metering, and flight-recorder diagnostics (`src/trace.rs`). Credential resolution retains typed rotation errors through request and startup-seed paths; terminal invalid-credential errors disable the account before fallback. OAuth completion disables an account rejected by immediate credential resolution and reports `reauthorization_required`, not success. Expiry-derived refresh scheduling uses up to a five-minute lead, reduced proportionally to half the remaining lease for leases shorter than ten minutes; the calculated deadline is retained while the expiry/refresh-hint identity is unchanged, and claims do not move that deadline, so short leases still refresh before expiry without sliding on each check.
...
- **Credential refresh and plugin runtime**:
  - `src/credential_refresh.rs` tests cover proactive rotation, ensure an unchanged four-minute lease rotates at its originally scheduled deadline, and ensure a successfully refreshed three-minute lease is not immediately due again.
  - `tests/plugin_auth_retry.rs`: Covers request-path terminal `credential_expired` disabling the revoked account and falling back, startup-seed disabling, auth retries, and scheduled refresh behavior.
  - `src/admin.rs` credential-enrollment regression tests cover OAuth completion disabling an immediately rejected credential, returning `reauthorization_required`, and avoiding a successful-enrollment audit.
