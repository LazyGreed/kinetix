...
- **Opaque Provider State (`src/opaque_state.rs`)**:
  ...
  - Cross-model continuation: an adapter that has a documented placeholder for non-portable continuation state returns it from `Adapter::opaque_state_placeholder` (default `None`; Gemini returns the provider's `skip_thought_signature_validator` sentinel). On a `strip_with_warning` Route the pipeline substitutes that placeholder onto the specific stored-but-incompatible historical call instead of stripping it; never-captured ids are left untouched, and `reject`/direct-target policies still refuse first. The sentinel lives on the adapter, never in the pipeline core.
