...
- **Core Pipeline & Router (`src/pipeline.rs`, `src/router.rs`, `src/registry.rs`)**:
  ...
  - Execution engine: handles upstream target dispatch, credential leasing from pools, streaming-safe failovers, response streaming, usage metering, and flight-recorder diagnostics (`src/trace.rs`). Credential resolution retains typed rotation errors through the request path; terminal invalid-credential errors disable the account before fallback. Expiry-derived refresh scheduling uses up to a five-minute lead, reduced proportionally to half the remaining lease for leases shorter than ten minutes, so successful short-lived rotations are not immediately due again.
...
- **Credential refresh and plugin runtime**:
  - `src/credential_refresh.rs` tests cover proactive rotation and ensure a successfully refreshed three-minute lease is scheduled in the future rather than immediately due again.
  - `tests/plugin_auth_retry.rs`: Covers request-path terminal `credential_expired` during `resolve()` disabling the revoked account and successfully falling back, as well as auth retries and scheduled refresh behavior.
